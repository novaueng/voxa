...
describe("Alexa directives", () => {
    ...
    describe("RenderTemplate", () => {
        ...
        it("should support adding a template directly", async () => {
            app.onIntent("YesIntent", () => {
                const template = new src_1.DisplayTemplate("BodyTemplate1");
                return {
    ...
    describe("HomeCard", () => {
        it("should be usable from the directives", async () => {
            app.onIntent("YesIntent", {
                directives: [new src_1.HomeCard("Card")],
                to: "die",
            });
            const reply = await alexaSkill.execute(event);
            chai_1.expect(reply.response.card).to.deep.equal({
        ...
        it("should render faile if variable doesn't return a card like object", async () => {
            ...
            chai_1.expect(reply.speech).to.include("An unrecoverable error");
        });
    ...
    describe("DialogDelegate", () => {
        ...
        it("should render a DialogDelegate directive with slot values", async () => {
            app.onIntent("GreetingIntent", (voxaEvent) => ({
    ...
    describe("DialogElicitSlot", () => {
        ...
        it("DialogElicitSlot no slotToElicit error", async () => {
            ...
            app.onError((request, error) => {
                chai_1.expect(error.message).to.equal("slotToElicit is required for the Dialog.ElicitSlot directive");
            });
        ...
        it("DialogElicitSlot transition error", async () => {
            ...
            app.onError((request, error) => {
                chai_1.expect(error.message).to.equal("You cannot transition to a new intent while using a Dialog.ElicitSlot directive");
            });
            await alexaSkill.execute(dialogStateEvent);
        });
    });
    describe("DialogElicitSlot After Completion", () => {
        ...
        it("DialogElicitSlot should return an error because dialog is complete", async () => {
            ...
            app.onError((request, error) => {
                chai_1.expect(error.message).to.equal("Intent is missing dialogState or has already completed this dialog and cannot elicit any slots");
            });
            await alexaSkill.execute(dialogStateEvent);
        });
    });
