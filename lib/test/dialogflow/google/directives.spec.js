...
describe("Google Assistant Directives", () => {
    ...
    beforeEach(() => {
        ...
        dialogflowAgent = new dialogflow_1.GoogleAssistantPlatform(app);
    ...
    describe("Context", () => {
        it("should add an output context", async () => {
            ...
            chai_1.expect(reply.outputContexts).to.deep.equal([
                {
                    lifespanCount: 5,
                    name: "projects/project/agent/sessions/1525973454075/contexts/DONE_YES_NO_CONTEXT",
                    parameters: undefined,
                },
                {
                    lifespanCount: 10000,
                    name: "projects/project/agent/sessions/1525973454075/contexts/attributes",
                    parameters: {
    ...
    describe("MediaResponse", () => {
        ...
        it("should throw an error if trying to add a MediaResponse without a simpleResponse first", async () => {
            const reply = new dialogflow_1.DialogflowReply();
            const googleAssistantEvent = new dialogflow_1.GoogleAssistantEvent(event);
            const mediaResponse = new dialogflow_1.MediaResponse(mediaObject);
            let error = null;
            try {
            ...
            chai_1.expect(error.message).to.equal("A simple response is required before a dialogflowMediaResponse");
        });
    });
    describe("Carousel", () => {
        it("should not add a carousel if the event has no SCREEN_OUTPUT", async () => {
            ...
            event.originalDetectIntentRequest.payload.surface.capabilities = [];
            const reply = await dialogflowAgent.execute(event);
            chai_1.expect(reply.payload.google.systemIntent).to.be.undefined;
        });
    ...
    describe("CompletePurchase Directive", () => {
        it("should add a CompletePurchase response", async () => {
            const googleCompletePurchaseOptions = {
    ...
    describe("Table Directive", () => {
        ...
        it("should not add a Table Response if no screen output", async () => {
            ...
            event.originalDetectIntentRequest.payload.surface.capabilities = [];
            const reply = await dialogflowAgent.execute(event);
            chai_1.expect(reply.hasDirective("Table")).to.be.false;
        });
    ...
    describe("BrowseCarousel", () => {
        it("should not include a browse carouse if no screen output", async () => {
            ...
            event.originalDetectIntentRequest.payload.surface.capabilities = [];
            const reply = await dialogflowAgent.execute(event);
            chai_1.expect(reply.hasDirective("BrowseCarousel")).to.be.false;
        });
        it("should include a new surface directive", async () => {
            app.onIntent("LaunchIntent", {
                dialogflowBrowseCarousel: {},
